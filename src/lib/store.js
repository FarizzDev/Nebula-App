...
// ── Materi ────────────────────────────────────────────────────
export async function getMateriList()  { return (await get(K.MATERI)) || [] }
...
// ── Reminders ─────────────────────────────────────────────────
export async function getReminders() { return (await get(K.REMINDERS)) || [] }
...
export async function toggleReminderDone(id) {
  const list = (await getReminders()).map(r => r.id === id ? { ...r, done: !r.done } : r)
  await set(K.REMINDERS, list)
  return list
}
...
export async function saveSettings(s) {
  ...
  // Sync ke Preferences keys yang dipakai gemini.js
  if (s.apiKey !== undefined) await Preferences.set({ key: 'nebula_api_key', value: s.apiKey })
  if (s.model  !== undefined) await Preferences.set({ key: 'nebula_model',   value: s.model })
  return merged
}

// ── Migration dari localStorage (jalankan sekali) ─────────────
export async function migrateFromLocalStorage() {
  const migrated = await get('nebula_migrated')
  if (migrated) return

  const keys = [K.MATERI, K.REMINDERS, K.SETTINGS]
  for (const key of keys) {
    const val = localStorage.getItem(key)
    if (val) await Preferences.set({ key, value: val })
  }
  // Migrate chat histories
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i)
    if (k?.startsWith(K.CHAT)) {
      const val = localStorage.getItem(k)
      if (val) await Preferences.set({ key: k, value: val })
    }
  }
  await set('nebula_migrated', true)
  console.log('[Store] Migrated from localStorage')
}
