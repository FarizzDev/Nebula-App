...
export const AVAILABLE_MODELS = [
  { id: 'gemini-2.5-flash-preview-04-17', label: 'Gemini 2.5 Flash (Recommended)' },
  { id: 'gemini-2.5-pro-preview-03-25',   label: 'Gemini 2.5 Pro (Smarter, slower)' },
  { id: 'gemini-2.0-flash-lite',          label: 'Gemini 2.0 Flash Lite (Fastest)' },
]
export const DEFAULT_MODEL = AVAILABLE_MODELS[0].id

async function getApiKey() {
  const { value } = await Preferences.get({ key: 'nebula_api_key' })
  return value || ''
}
async function getModel() {
  const { value } = await Preferences.get({ key: 'nebula_model' })
  return value || DEFAULT_MODEL
}

export async function sendToGemini({ messages, systemInstruction, useSearch = false, onStream }) {
  const apiKey = await getApiKey()
  if (!apiKey) throw new Error('API key belum diset. Pergi ke Pengaturan dulu!')
...
export function buildSystemPrompt({ materiContext, userInterests = 'teknologi dan programming', mode = 'general' } = {}) {
  ...
  const map  = {
    ...
    quiz:    `${base}\n\nBuat soal yang menguji pemahaman bukan hafalan. Kembalikan HANYA JSON array valid, tanpa teks lain, tanpa markdown. Format: [{"id":"1","pertanyaan":"...","tipe":"pilihan_ganda","pilihan":["A. ...","B. ...","C. ...","D. ..."],"jawaban_benar":"A. ...","penjelasan":"..."}]`,
...
export async function diskusiJawaban({ soal, jawabanUser, jawabanBenar, penjelasan, onStream }) {
  return sendToGemini({
    messages: [{ role: 'user', parts: [{ text: `Soal: ${soal}\nJawaban saya: ${jawabanUser}\nJawaban benar: ${jawabanBenar}\nPenjelasan: ${penjelasan}\n\nAnalisis jawaban saya secara detail!` }] }],
    systemInstruction: buildSystemPrompt({ mode: 'discuss' }),
    onStream,
  })
}

export async function chat({ history, userMessage, materiContext, useSearch = false, onStream }) {
  return sendToGemini({
