...
export default function Chat() {
  ...
  useEffect(() => { setMateriList(getMateriList()); setHistory(getChatHistory('general')) }, [])
  useEffect(() => { setHistory(getChatHistory(selectedMateri)) }, [selectedMateri])
  ...
  async function handleSend() {
    ...
    try {
      ...
      setHistory(final); saveChatHistory(selectedMateri, final)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
      <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(10,10,26,0.8)', backdropFilter: 'blur(10px)' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ fontSize: 11, color: 'var(--color-text-3)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        ...
        <button onClick={() => { if(confirm('Hapus chat?')) { setHistory([]); saveChatHistory(selectedMateri, []) } }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, color: 'var(--color-text-3)' }}>🗑️</button>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
        {history.length === 0 && (
          <div style={{ textAlign: 'center', padding: '32px 0' }} className="stagger">
            ...
            <p style={{ fontSize: 13, color: 'var(--color-text-3)', marginBottom: 16 }}>Tanya apa aja, siap bantu!</p>
        ...
        {history.map((msg, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', marginBottom: 12, gap: 8 }} className="animate-fade-up">
            ...
            <div style={{ maxWidth: '82%', padding: '9px 13px', borderRadius: 14, fontSize: 13, lineHeight: 1.5, background: msg.role === 'user' ? 'rgba(59,130,246,0.15)' : 'var(--color-surface-2)', border: `1px solid ${msg.role === 'user' ? 'rgba(59,130,246,0.3)' : 'var(--color-border)'}`, color: 'var(--color-text)' }}>
      ...
      <div style={{ padding: '10px 16px', borderTop: '1px solid var(--color-border)', background: 'rgba(10,10,26,0.8)', backdropFilter: 'blur(10px)' }}>
