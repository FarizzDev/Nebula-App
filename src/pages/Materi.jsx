...
export default function Materi() {
  ...
  const handleSearchChange = useCallback(
    debounce(() => {
      setFinalSearchTerm(searchRef.current?.value || "");
    }, 500),
    [],
  );

  const filtered = useMemo(() => {
    if (!finalSearchTerm) return list;
    return list.filter(
      (m) =>
        m.judul.toLowerCase().includes(finalSearchTerm.toLowerCase()) ||
        (m.tags || "").toLowerCase().includes(finalSearchTerm.toLowerCase()),
    );
  }, [list, finalSearchTerm]);

  async function handleSave(form) {
    const isEdit = modal && modal !== "add";
    const updated = await saveMateri({
      ...(isEdit ? modal : {}),
      ...form,
      id: isEdit ? modal.id : Date.now().toString(),
    });
    setList(updated);
    setModal(null);
    await hapticSuccess();
  ...
  async function handleFetchAI() {
    ...
    try {
      let full = "";
      await fetchMateriFromTopic(aiTopic, (c) => {
        full += c;
        setStreamText((t) => t + c);
      });
      const updated = await saveMateri({
    ...
    <div style={{ padding: "20px 16px", maxWidth: 600, margin: "0 auto" }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12, color: "#8090a8", marginTop: 1 }}>
            {list.length} tersimpan
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <input
        ...
        placeholder="🔍  Cari materi atau tag..."
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "52px 0" }}>
          ...
          <p style={{ fontSize: 13, color: "#8090a8" }}>
            Tambah manual, upload, atau minta AI cariin
          </p>
        </div>
      ) : (
        filtered.map((m) => (
          ...
          >
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    background: addMode === k ? undefined : "transparent",
                    color: addMode === k ? undefined : "var(--color-text-3)",
                  }}
                >
                  {l}
                </button>
            ...
            <div style={{ padding: 16 }}>
              ...
              {addMode === "ai" && (
                ...
                >
                  <p style={{ fontSize: 13, color: "#8090a8" }}>
                    Masukkan topik, AI akan cariin dari internet
                  </p>
                  <input
                    value={aiTopic}
                    onChange={(e) => setAiTopic(e.target.value)}
                    onKeyDown={(e) =>
                      e.key === "Enter" && !loading && handleFetchAI()
                    }
                    placeholder="Hukum Newton, Integral..."
                    style={inputS}
                    onFocus={(e) =>
                      (e.target.style.borderColor = "rgba(96,165,250,0.5)")
                    }
