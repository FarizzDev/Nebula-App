...
import { requestNotifPermission, scheduleNotifications, getNotifStatus, hapticMedium, hapticSuccess } from '../lib/notifications'
...
export default function Reminder() {
  ...
  async function handleSave() {
    ...
    const updated = await saveReminder({ ...form, id: Date.now().toString(), done: false })
  ...
  const now          = new Date()
  const filtered     = reminders.filter(r => filter==='semua'?true:filter==='selesai'?r.done:!r.done)
  const sorted       = [...filtered].sort((a,b)=>{ if(!a.deadline)return 1; if(!b.deadline)return -1; return new Date(a.deadline)-new Date(b.deadline) })
  ...
  const notifInfo = {
    default:     { label:'🔔 Aktifkan Notifikasi', color:'var(--color-neon-2)', bg:'rgba(59,130,246,0.07)', border:'rgba(59,130,246,0.3)', hint:'tap untuk aktifkan', disabled:false },
    granted:     { label:'✅ Notifikasi Aktif',    color:'var(--color-success)', bg:'rgba(52,211,153,0.07)', border:'rgba(52,211,153,0.3)', hint:'H-1 hari, H-1 jam, tepat deadline', disabled:true },
    denied:      { label:'🚫 Notifikasi Diblokir', color:'var(--color-danger)',  bg:'rgba(248,113,113,0.07)',border:'rgba(248,113,113,0.3)',hint:'izinkan di pengaturan app', disabled:true },
    unsupported: { label:'❌ Tidak Didukung',      color:'var(--color-text-3)',  bg:'transparent',           border:'var(--color-border)',   hint:'', disabled:true },
  }[notifStatus] || {}
    ...
    <div style={{ padding:'20px 16px', maxWidth:600, margin:'0 auto' }}>
      <div style={{ marginBottom:14 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
          <div>
            ...
            <p style={{ fontSize:12, marginTop:2, color:overdueCount>0?'var(--color-danger)':'#8090a8' }}>
              {activeCount} aktif{overdueCount>0?` · ⚠️ ${overdueCount} terlambat`:''}
            </p>
          </div>
          <button onClick={()=>setShowAdd(true)} className="btn-primary" style={{ padding:'8px 18px', borderRadius:10, fontSize:13, fontWeight:700 }}>+ Tambah</button>
        </div>

        <button onClick={handleEnableNotif} disabled={notifInfo.disabled}
          style={{ marginTop:10, width:'100%', padding:'9px 13px', borderRadius:10, fontSize:12, fontWeight:500,
            cursor:notifInfo.disabled?'default':'pointer', border:`1px solid ${notifInfo.border}`,
            background:notifInfo.bg, color:notifInfo.color, textAlign:'left' }}>
          {notifInfo.label}
          {notifInfo.hint && <span style={{ color:'#8090a8', fontSize:11, marginLeft:6 }}>— {notifInfo.hint}</span>}
        </button>
      </div>
      ...
      <div style={{ display:'flex', gap:3, marginBottom:14, background:'var(--color-surface)', borderRadius:10, padding:4 }}>
        {[['aktif','Aktif'],['selesai','Selesai'],['semua','Semua']].map(([k,l])=>(
          <button key={k} onClick={()=>setFilter(k)} className={filter===k?'btn-neon':''}
      ...
      {sorted.length === 0 ? (
        ...
        return (
          <div key={r.id} className="glass" style={{ borderRadius:12, padding:'12px 14px', marginBottom:8, display:'flex', alignItems:'flex-start', gap:12, opacity:r.done?0.5:1, border:`1px solid ${overdue?'rgba(248,113,113,0.3)':'var(--color-border)'}`, boxShadow:overdue?'0 0 12px rgba(248,113,113,0.08)':'none' }}>
            <button onClick={()=>handleToggle(r.id)}
              style={{ width:22, height:22, borderRadius:'50%', marginTop:1, flexShrink:0, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, transition:'all 0.2s', border:`2px solid ${r.done?'var(--color-success)':'var(--color-border-2)'}`, background:r.done?'var(--color-success)':'transparent', color:'white', boxShadow:r.done?'0 0 8px rgba(52,211,153,0.4)':'none' }}>
              {r.done && '✓'}
            </button>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:2 }}>
                <div style={{ width:7, height:7, borderRadius:'50%', background:pColor, flexShrink:0, boxShadow:`0 0 5px ${pColor}` }} />
                <p style={{ fontSize:14, fontWeight:500, color:'var(--color-text)', textDecoration:r.done?'line-through':'none', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{r.judul}</p>
              </div>
              {r.catatan && <p style={{ fontSize:12, color:'#8090a8', marginBottom:2, paddingLeft:13 }}>{r.catatan}</p>}
              {dl && <p style={{ fontSize:11, paddingLeft:13, color:overdue?'var(--color-danger)':today?'var(--color-warning)':'#8090a8' }}>
                {overdue?'⚠️ Terlambat · ':today?'⏰ Hari ini · ':'📅 '}
                {dl.toLocaleDateString('id-ID',{weekday:'short',day:'numeric',month:'short',year:'numeric'})}
              </p>}
            </div>
            <button onClick={()=>handleDelete(r.id)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--color-text-3)', fontSize:14, padding:4, flexShrink:0 }}>✕</button>
          </div>
        )
      })}
        ...
        <div style={{ position:'fixed', inset:0, zIndex:100, display:'flex', alignItems:'flex-end', justifyContent:'center', padding:16 }}>
          ...
          <div className="glass animate-fade-up" style={{ position:'relative', width:'100%', maxWidth:500, borderRadius:20, border:'1px solid var(--color-border-2)' }}>
            ...
            <div style={{ padding:16, display:'flex', flexDirection:'column', gap:10 }}>
              ...
              <div>
                <p style={{ fontSize:12, color:'#8090a8', marginBottom:5 }}>Deadline</p>
              ...
              <div>
                <p style={{ fontSize:12, color:'#8090a8', marginBottom:5 }}>Prioritas</p>
                <div style={{ display:'flex', gap:6 }}>
                  {[['rendah','🟢'],['normal','🟡'],['tinggi','🔴']].map(([p,e])=>(
                    <button key={p} onClick={()=>setForm(f=>({...f,prioritas:p}))} className={form.prioritas===p?'btn-neon':''}
              ...
              <button onClick={handleSave} className="btn-primary" style={{ width:'100%', padding:13, borderRadius:11, fontSize:14, fontWeight:700 }}>Simpan Reminder</button>
            </div>
          </div>
        </div>
      )}
